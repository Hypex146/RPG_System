...
public class CommandLvl implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length==0) {
			sender.sendMessage("Ваш уровень: "
		...
		if (args[0].equals("up")) {
			if (mainPlugin.getEconomy().getBalance((OfflinePlayer)sender)>=500) {
				...
				mainPlugin.getPlayerDataMap().get(player.getUniqueId().toString()).setPlayerLevel(
						mainPlugin.getPlayerDataMap().get(player.getUniqueId().toString()).getPlayerLevel()+1);
