...
public class CommandLvl implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length==0) {
			sender.sendMessage("Ваш уровень: "+mainPlugin.playerDataMap.get(sender.getName()).getPlayerLevel());
			return true;
		}
		if (args[0].equals("up")) {
			if (mainPlugin.getEconomy().getBalance((OfflinePlayer)sender)>=500) {
				...
				mainPlugin.playerDataMap.get(sender.getName()).setPlayerLevel(
						mainPlugin.playerDataMap.get(sender.getName()).getPlayerLevel()+1);
